...
def context():
    ...
    return {"service": service, "db": repo, "totals": {}}
...
@then(parsers.parse("el mes {month_id} debe sumar {expected_total:d} euros"))
def check_month_total(context, month_id, expected_total):
    context["totals"] = context["service"].total_by_month()
    total_actual = context["totals"].get(month_id, 0)
